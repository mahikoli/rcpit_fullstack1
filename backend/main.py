        ...
        with engine.connect() as conn:
            ...
            for row in result:
                issues.append({
                    ...
                    "equipment_type": row._mapping.get("equipment_type"),
                    "lab_name": row._mapping.get("lab_name"),
                    "room_name": row._mapping.get("room_name"),
                    "equipment_name": row._mapping.get("equipment_name"),
                    "equipment_id": row._mapping.get("equipment_id"),
                    "description": row._mapping.get("description"),
                    "user_name": row._mapping.get("user_name"),
                    "email": row._mapping.get("email"),
                    "prn": row._mapping.get("prn"),
                    "status": row._mapping.get("status"),
                    "priority": row._mapping.get("priority"),
                    "created_at": str(row._mapping.get("created_at")),
                    "technician_id": row._mapping.get("technician_id"),
                    "technician_name": row._mapping.get("technician_name"),
                    "estimated_days": row._mapping.get("estimated_days"),
                    "technician_comment": row._mapping.get("technician_comment"),
                    "issue_subtype": row._mapping.get("issue_subtype"),
                    "reporter_count": row._mapping.get("reporter_count") if row._mapping.get("reporter_count") is not None else 1,
                    "student_dept": row._mapping.get("student_dept") if row._mapping.get("student_dept") is not None else "IT",
                    "is_escalated": bool(row._mapping.get("is_escalated")) if row._mapping.get("is_escalated") is not None else False
                })
            return issues
    except Exception as e:
